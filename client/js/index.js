(() => {
  ...
  function getPosInclideStr(what = '', where = '') {
    return where.toLocaleLowerCase().search(what.toLocaleLowerCase());
  }

  // автокомплит
  function autoComplite(selector, data) {
    const inputs = document.querySelectorAll(selector);

    inputs.forEach(input => {
      input.classList.add('autocomplite__input');
      const wrapper = document.createElement('div');
      wrapper.classList.add('autocomplite__wrapper');
      input.parentNode.insertBefore(wrapper, input);
      wrapper.appendChild(input);

      const list = document.createElement('div');
      list.classList.add('autocomplite__list');
      wrapper.append(list);
      ...
      input.addEventListener('input', () => {
        ...
        data.forEach((dataItem, index) => {
          ...
          item.classList.add('autocomplite__item');
  ...
  async function createClientsHeader() {
  ...
  document.addEventListener('DOMContentLoaded', async () => {
    ...
    headerContainer = await createClientsHeader();
    ...
    clients.forEach(el => {
      clientsFio.push(`${el.surname} ${el.name} ${el.lastName}`);
    })

    autoComplite('#autocomplite', clientsFio);
    ...
    headerContainer.headerInput.addEventListener('input', () => {
      console.log(`input: ${headerContainer.headerInput.value}`);
    })

    headerContainer.headerInput.addEventListener('change', () => {
      console.log(`change: ${headerContainer.headerInput.value}`);
      renderAfterFilter();
    })
